...
<img src="frontend/public/brand/logo-light.png" alt="ШАҒРА логотипі" width="92">
...
> **ШАҒРА қызметкер профилін, келесі грейд талаптарын және даму тарихын тексерілетін мансаптық қадам жоспарына айналдырады.** Әр іс-шараның әсерін орындамай тұрып көруге және салыстыруға болады.

Қызметкер дағды алшақтықтары мен негізделген ұсыныстарды көреді, ал HR команда жиынтығы мен қауіпсіз профиль импортын алады. Барлық есептеуді нақты API орындайды; AI деректерді алмастырмай, нұсқаларды реттейді.

<div align="center">

[🚀 Іске қосу](#жылдам-іске-қосу) · [✨ Мүмкіндіктер](#мүмкіндіктер) · [🤖 AI режимдері](#қазылар-алқасына-тексеру) · [🏗 Архитектура](#архитектура) · [📚 Құжаттама](docs/README.md)

</div>

## Өнім жұмыс үстінде

![Қызметкер профилі мен мансаптық мақсаты](docs/validation/ui/simple/live-profile-1440-dark.png)

| Мансаптық іс-шараларды салыстыру | HR талдауы |
|---|---|
| ![Нұсқаларды салыстыру](docs/validation/ui/simple/live-preview.png) | ![HR панелі](docs/validation/ui/simple/live-hr-1440-dark.png) |

## Несімен ерекшеленеді

| Жалпы кеңестің орнына контекст | Тексерілетін нәтиже | Үш демо режимі |
|---|---|---|
| Грейдті, нақты алшақтықтарды және қызметкер тарихын ескереді | Ұсыныс негізін және растауға дейінгі дағды өзгерісін көрсетеді | OpenAI API, жергілікті Ollama немесе кілтсіз дайын fallback |
...
## Қазылар алқасына тексеру

[Сарапшыға арналған нұсқаулықта](docs/JURY.md) үш режим бар: жеке берілетін тестілік кілтпен OpenAI, аккаунтсыз жергілікті Ollama және дайын офлайн сценарий. Офлайн режимге кілт қажет емес, ақылы сұраулар өшірілген:

```bash
docker compose --env-file config/jury-offline.env -p shagra-jury -f compose.yaml -f compose.offline.yaml up -d --build app
```

**http://localhost:8081** мекенжайын ашыңыз. Бөлек деректер томы негізгі қолданбаның күйін сақтайды. Бұл — **ережелер бойынша іріктеу**, нақты уақыттағы AI емес. [Импортқа арналған төрт дайын профиль](data/demo-import/README.md) пайдалы қадамды, орындалған мақсатты, соңғы грейдті және белгісіз дағды бағасын көрсетеді.
...
Бұл — жергілікті демонстрациялық MVP. Дағдылардың талаптарға сәйкестігі қызметкерді жоғарылату туралы шешім емес; ұйымдастырушылардың ресми жинағымен үйлесімділік тексерілмеген. Серверлік негіздемелер әзірге орыс тілінде; HR жауаптарының бір бөлігінде рөл атаулары жоқ. `docs/validation/ui/redesign` және `start-design.sh` — тарихи алдын ала қарау материалдары, жұмыс қолданбасына қажет емес. `.env`, runtime дерекқорлары мен орнатылған тәуелділіктер Git-ке қосылмайды.
