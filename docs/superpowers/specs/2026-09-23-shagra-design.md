...
Дата: 23 сентября 2026. Версия: 1.1. Трек: Halyk Bank, Career Quest.

Уточнение стека: frontend использует Tailwind CSS 4; основной AI-провайдер — OpenAI API с GPT-4.1 mini, Ollama доступна как локальная опция. Бюджет пользователя на API — $50. Изменена спецификация, платные запросы при проектировании не выполнялись.
...
Макеты: спокойный светлый интерфейс, тёмно-зелёный основной цвет, янтарный для примерки, нейтральные таблицы. Цвет не является единственным носителем смысла. Все действия доступны клавиатурой, поля имеют подписи. При ширине 1280 px профиль и карточки рядом; при ширине <900 px одна колонка. Обязательны единые токены цвета/типографики/отступов, хорошо различимые before/after, выразительные карточки и качественные loading/empty/error состояния. Анимации 150–200 ms с поддержкой prefers-reduced-motion; действия не зависят только от hover.

Tailwind 4 поддерживает современные браузеры: Chrome 111+, Safari 16.4+, Firefox 128+; для Edge проверять актуальную Chromium-версию. Это не обещание поддержки всех исторических браузеров. Проверить текущие Chromium/Firefox/WebKit через Playwright на ширинах 390, 768 и 1280 px. Responsive layout не должен скрывать функции HR/импорта на телефоне. [Официальная совместимость](https://tailwindcss.com/docs/compatibility).

## 4. Архитектура и стек

### 4.1. Выбранная конструкция

React + TypeScript + Vite + Tailwind CSS 4; backend Python 3.12 + FastAPI + Pydantic 2; стандартный sqlite3; httpx для обращения к LLM; pytest и Playwright. Один backend-процесс, один файл БД; основной AI — OpenAI API, отдельный процесс Ollama запускается только в локальном режиме. FastAPI раздаёт собранный frontend и /api/v1 с одного origin. Прокси Vite нужен только при разработке.

Зависимости frontend: react, react-dom, react-router-dom, vite, typescript, tailwindcss, @tailwindcss/vite; тестовые @playwright/test. Подключить официальный Vite plugin и `@import "tailwindcss";` в app.css; токены объявить через @theme, повторяющиеся карточки/кнопки оформить React-компонентами. Дополнительный CSS — только там, где utility-классы неудобны. Не использовать Play CDN, Sass, динамическую конкатенацию имён классов или автоматически переносить конфигурацию Tailwind 3. [Установка через Vite](https://tailwindcss.com/docs/installation/using-vite). Backend: fastapi, uvicorn, pydantic, httpx, python-multipart, itsdangerous; pytest для тестов. Конкретные совместимые версии фиксируются lock-файлами при первой установке; в финальной сдаче плавающих версий нет. Не устанавливать новые пакеты для функций стандартной библиотеки.
...
flowchart LR
  ...
  REC --> CORE
  REC --> LLM[RankingProvider]
  LLM --> OPENAI[OpenAI Responses API]
  LLM --> LOCAL[Ollama: локальная опция]
...
backend/
  app/
    ...
    recommendations/        # А2: candidates.py, service.py, validation.py,
                            #     explanations.py, provider.py, openai_provider.py,
                            #     ollama.py, prompt.txt
    bootstrap.py            # А1
    main.py                 # А1
  tests/core/               # А1
  tests/api/                # А1
...
Основной провайдер: OpenAI Responses API, POST https://api.openai.com/v1/responses, модель gpt-4.1-mini-2025-04-14. Параметры: stream=false, store=false, max_output_tokens=512, temperature=0; JSON Schema передаётся в text.format={type:"json_schema",name:"ranked_choices",strict:true,schema:...}. В schema у каждого объекта additionalProperties=false, все поля обязательны; enum задаёт aliases и reason_codes. Инструкция передаётся через instructions, JSON кандидатов — через input. Успешный ответ должен иметь status=completed; извлекать output_text из message/content элементов output, отклонять refusal, incomplete и отсутствие текста. HTTP-клиент httpx, Authorization: Bearer из серверного OPENAI_API_KEY, без автоматических повторов. [Модель и цена](https://developers.openai.com/api/docs/models/gpt-4.1-mini), [Structured Outputs](https://developers.openai.com/api/docs/guides/structured-outputs).

Локальная опция при AI_PROVIDER=ollama: /api/chat, model=qwen2.5:1.5b, stream=false, format=JSON Schema, temperature=0, num_predict=512, keep_alive=30m. Провайдер выбирается при старте, автоматического второго платного/локального запроса после ошибки нет: сразу deterministic fallback.

Системный prompt хранится версионированно. Максимум восемь кандидатов, description обрезается до 300 символов, суммарный JSON вход <=12 KiB. Полные CSV, ФИО, секреты и чужие профили не передаются. Внешнему API передаётся минимальный контекст одного сотрудника без employee_id и исходных history_id: грейд/цель, необходимые уровни, агрегаты участия, aliases и описания кандидатов.
...
LLM timeout=8 секунд включает ожидание слота. Общий бюджет HTTP-рекомендации — 9.5 секунды; отмена вызова при дедлайне. Конкурентность: OpenAI максимум три вызова, Ollama один. Если слот недоступен в оставшийся бюджет — fallback. Повторных LLM-вызовов внутри одного запроса нет. OpenAI 429 → fallback rate_limited, 401/403/5xx/нет ключа → unavailable; отказ модели → invalid_output. Ключ никогда не подставляется в сообщение ошибки.

Кэш в памяти на 15 минут, не более 500 записей. Ключ: employee_id, employee_version, dataset_version, sorted(excluded_event_ids), preferred_type, prompt_version, provider, model. После изменения версии старый результат не выдаётся. Перед возвратом перечитать версии: если изменились, вернуть 409 STALE_CONTEXT, UI обновляет профиль. Ошибки/таймауты кэшируются максимум на 15 секунд, чтобы не закреплять сбой надолго.

source: llm | deterministic_fallback; cache_hit: boolean; fallback_reason: null | timeout | unavailable | invalid_output | busy | context_too_large | rate_limited | call_limit. Если сериализованный вход превышает 12 KiB, сначала удалить description у всех кандидатов; если превышение осталось, не посылать усечённый JSON, а вернуть fallback context_too_large. Не называть fallback AI-рекомендацией. В UI он означает доступность системы при сбое, а не выполнение требования «работает LLM».

GPT-4.1 mini выбран для ограниченной задачи ранжирования с JSON-ответом и без отдельного reasoning-этапа. Это исходный кандидат, не заявление о превосходстве над всеми моделями. А2 в первые 30 минут проверяет доступность модели для API-проекта, квоту, качество и задержки; $50 не гарантируют доступ к любой модели или отсутствие rate limits. Ollama выбирается для локального режима отдельно. Если рабочей модели нет, это явно незакрытый критерий MVP; красивый fallback его не заменяет.

### 8.5. Бюджет OpenAI

По опубликованной цене GPT-4.1 mini: $0.40 за 1 млн входных и $1.60 за 1 млн выходных токенов. При предположении 2500 входных и 300 выходных токенов: (2500*0.40 + 300*1.60)/1_000_000 = $0.00148 за вызов; 1000 вызовов ≈$1.48, 10 000 ≈$14.80. Это оценка по указанной длине, без скидки кэша и без хостинга/других API; фактическое usage может отличаться. Источник цены — карточка модели в §8.2, проверена 23.09.2026.

Для пятичасового MVP достаточно коротких запросов на одного сотрудника; не отправлять 200 профилей на каждый клик и не использовать модель для HR-агрегатов. Предел приложения AI_MAX_PAID_CALLS=2000 попыток за весь срок жизни runtime, включая ошибки и startup probe; cache hits не учитываются. Это ограничение числа вызовов, не обещание жёсткого долларового лимита аккаунта. А2 резервирует попытку до запроса под async-lock, сохраняет счётчик атомарной заменой /app/runtime/ai-usage.json; после рестарта он не обнуляется. Файл содержит attempts, input_tokens, output_tokens, known_cost_usd, unknown_usage_attempts. При потере usage из-за timeout увеличивать unknown_usage_attempts, не считать стоимость нулевой. При повреждении файла блокировать новые платные вызовы с call_limit до исправления оператором. Отчёты live-eval используют тот же лимитер/файл в контейнере. Пользовательский бюджет $50 общий; прочие процессы и ключи этим счётчиком не контролируются.

Параметры ключа заполняются только в локальном .env сервера; файл исключён из git и Docker build context. В VITE_* ключ не передавать. При проектировании не нужны ключ или платные вызовы.
...
Constructor: RecommendationService(provider: RankingProvider, model: str, prompt_version: str="1"). RankingProvider — Protocol с provider_id: str и async rank(payload: dict, schema: dict, timeout_seconds: float) -> dict. Реализации: OpenAIProvider и OllamaProvider; модель/ключ/URL — параметры constructor провайдера из env. Возвращается распарсенный объект choices; usage учитывается внутри OpenAIProvider. FakeProvider для unit-тестов реализует такую же сигнатуру и возвращает заданный объект/ошибку. Он используется только в тестах, не подменяет LLM в production пути.

А1 получает ctx из Repository и проверяет версии до/после recommend. А2 реализует ранжирование, кэш и timeout. Не делать HTTP-вызов из frontend прямо к OpenAI или Ollama.
...
|---|---|---|
| GET /health | Все | {status:"ok", dataset_version:int, model_status:"ready"/"loading"/"unavailable", provider:"openai"/"ollama", model:string} |
...
В режиме OpenAI минимальный контекст из §8.2 отправляется внешнему API; README явно сообщает об этом. Демонстрация и разработка используют собственную синтетику. Для данных организаторов соблюдать условия их использования: разрешение облачных LLM в TZ.txt не следует описывать как разрешение публично распространять датасет. Если условия конкретного кита запрещают внешнюю обработку, использовать AI_PROVIDER=ollama. Локальный режим не отправляет контекст OpenAI и не запускает его probe.

store=false не означает гарантированное отсутствие всех журналов у провайдера. Не заявлять zero retention без соответствующих настроек аккаунта; режим обработки сверять с [официальной документацией данных API](https://developers.openai.com/api/docs/guides/your-data). Ollama в локальном режиме в Docker-сети без публикации порта. Публичная телеметрия приложения отсутствует.

## 13. Запуск и измеряемые ограничения

Основная команда из корня: `docker compose up --build`. Адрес: http://localhost:8080. Docker Engine и Compose v2 — предварительные требования, явно указанные в README. Для настоящего AI в основном режиме заранее заполнить OPENAI_API_KEY в локальном .env; без ключа интерфейс запускается в честном fallback, а AI-проверка не считается пройденной. Первый запуск требует сети для зависимостей/образов; веса в режиме OpenAI не скачиваются.

Compose по умолчанию запускает только app (порт 127.0.0.1:${APP_PORT:-8080}). ollama/model-init включены в опциональный profile local-ai. Локальная команда: `AI_PROVIDER=ollama docker compose --profile local-ai up --build`; model-init ждёт Ollama и делает pull ${OLLAMA_MODEL:-qwen2.5:1.5b} в отдельный постоянный том. app не блокируется на readiness, health показывает loading/unavailable. ready означает успешный короткий schema-вызов выбранного provider в пределах timeout; для OpenAI probe — одна платная попытка при старте, учитываемая лимитером; GET /health сам платных запросов не делает. Последующие успешные рекомендации обновляют статус. Сбой любого provider сохраняет доступность основного интерфейса через fallback.
...
| AI_PROVIDER | openai; допустимо также ollama |
| OPENAI_API_KEY | Пусто; заполняется локально для AI через API |
| OPENAI_MODEL | gpt-4.1-mini-2025-04-14 |
| AI_MAX_PAID_CALLS | 2000 |
...
Основной режим OpenAI не требует локального GPU или загрузки весов; задержка зависит от API и сети, измеряется по §14. Для опционального Ollama рабочая цель — laptop с 8 GiB RAM или больше; фактические RAM/скорость измеряются. Ни для одного provider без проверки нельзя обещать AI <10 секунд.
...
| Q19 | Нет API-ключа / OpenAI 429 / 401 / timeout / refusal | Явный fallback, нет повторного платного вызова, секрет не отражён в ошибке |
| Q20 | Лимит 2000 попыток достигнут, затем restart | Новые платные вызовы блокируются; счётчик сохраняется; cache hit не расходует попытку |
| Q21 | Переключение openai→ollama | Нет OpenAI-запросов/probe в локальном режиме; в кэше разные ключи provider/model |
...
1. Проверить OpenAI API, ключ/квоту и формат ответа; параллельно создать acceptance-фикстуры. Для локального режима отдельно проверить Ollama.
2. Реализовать candidates, score и deterministic fallback по контракту; unit-тесты Q1–Q6.
3. Реализовать общий RankingProvider, OpenAI adapter и локальный Ollama adapter, prompt и JSON Schema; проверить Q7/Q8/Q16 и лимитер платных вызовов.
...
1. Создать Vite/React/Tailwind 4, api/types.ts по §9–10; сразу использовать примеры контрактов как browser route mocks в тестах.
...
|---|---|---|---|---|
| 0–20 | Контракты, каркас API, examples | OpenAI: key/probe, acceptance fixtures | Структура UI, API-типы, Tailwind | Единые имена/типы и известный статус модели |
...
| Выбранная модель плохо ранжирует | Live-проверка в начале; измеримое сравнение; смена model по результатам с проверкой новой цены, не скрытая подмена эвристикой |
| API/сеть не укладываются в задержку или дают 429 | Ранний benchmark, компактный prompt, кэш и timeout; для локальной опции отдельно измерить CPU. Fallback сохраняет доступность, но не закрывает AI-качество |
