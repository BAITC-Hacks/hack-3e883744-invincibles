...
Сервис получает готовый `EmployeeContext` от A1 и никогда не читает БД или не меняет профиль. A1 публикует `app.contracts` и пять функций `app.core` из §9.2 спецификации. `build_candidates(ctx, request)` использует только эти функции для допуска, примерки, цели и истории. `RecommendationService.recommend(ctx, request)` возвращает `RecommendationResult` из общего контракта. A1 проверяет версии snapshot до и после вызова и возвращает HTTP 409 при изменении.
