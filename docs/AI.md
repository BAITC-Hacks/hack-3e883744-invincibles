...
На 2026-09-23 локальные unit-тесты A2 проходят: 12 тестов, 6 подслучаев, без сети. Полный `backend/tests` проходит с `PYTHONUTF8=1` на Windows: 42 теста, 6 подслучаев. Независимые контрольные Q1–Q3 записаны в `data/acceptance/q1-q3.json` до настройки prompt и прошли с настоящей моделью `gpt-4.1-mini-2025-04-14`: первые шаги совпали с ожидаемыми, `source=llm`, три обязательных evidence присутствуют, время последнего прогона 1.6106 / 1.0451 / 1.9402 с. Минимальный schema-probe прошёл за 3.2834 с с usage 288 входных и 23 выходных токена.

Первый живой HTTP-прогон с prompt version 1 дал 9/30 валидных LLM-ответов (30%): модель часто возвращала четыре–пять choices вместо трёх. Валидатор отклонял весь ответ. Prompt version 2 явно передавал `required_choice_count` и aliases максимальной пользы. Он дал 28/30, затем 29/30 LLM-ответов; у одного запроса оставался `invalid_output`, но конкретный ответ не сохранялся. В prompt version 3 число choices дополнительно фиксируется в Structured Outputs через `minItems` и `maxItems`, которые [поддерживаются для обычных моделей](https://developers.openai.com/api/docs/guides/structured-outputs). Повторная серия на закоммиченной версии 3 и штатном `app.main:app` дала 30/30 валидных LLM-ответов, без fallback; для cache misses p50=1.6165 с, p95=2.1156 с, max=2.1356 с. Отдельная серия дала 30/30 cache hits, p95=0.0126 с. Критерии качества и задержки выполнены на этой серии; единичный прошлый сбой нельзя считать невозможным в будущем, поэтому серверный валидатор и fallback сохранены. Итоговые измерения — в `docs/validation/ai.json` и `latency.json`.

После интеграционного коммита A1 проверен обычный `app.main:app`: `/api/v1/health` сообщил `model_status=ready`, HTTP-рекомендации и cache hits прошли. Постоянный локальный ledger после всех проверок: 169 платных попыток, 180160 известных входных токенов, 10038 выходных, оценка известной стоимости $0.0881248, один вызов с неизвестным usage. Это оценка по тарифу ТЗ, а не выписка аккаунта. Docker/Compose и официальный датасет на этой машине не проверены.
