...
Последующее указание пользователя расширило дизайн переключаемой светлой темой, языками RU/KZ/EN и более простыми экранами. Эта версия внедрена в `frontend/src/app`, `pages`, `features`, `entities` и `shared`. IBM Plex Sans/Mono указанных выше версий теперь используются в production; локальные WOFF2 и лицензии находятся в `frontend/src/shared/styles/fonts`. CSS использует только включённые WOFF2, без ссылок на отсутствующие WOFF. Контурные SVG заданы в общем компоненте Icon; уведомления о лицензиях Lucide/Feather также сохранены рядом с ресурсами.
